...
For example, assuming a :class:`ParticleGroup` contains many different species of particles, one might want to select all particles of a given species that also have weight below some threshold, and then merge them to avoid tracking many small particles. This example is a supported feature in Reactions, as we expect that many reaction products with small weights are likely to be produced. 

In order to accommodate the above, Reactions offers a uniform interface for :class:`MarkingStrategy`, :class:`TransformationStrategy`, and :class:`TransformationWrapper` objects.
...
Reactions assumes that particles carry information of their contribution to various sources that need to be projected onto the grid. A common requirement in these situations is to reset the values of sources on the particles after projection. Reactions offers the ParticleDatZeroer transformation strategy that allows the user to accomplish this.
...
Another common requirement is the accumulation of particle properties cellwise. This is a requirement for finite volume methods (projection of sources) as well as general particle data analysis (weighted averages of quantities). Two classes of transformation strategies are provided in Reactions for this use.
...
Reaction implements a simplified merging algorithim from [VRANIC2015]_. It assumes that all particles being merged are of the same species (i.e. have the same mass) and that they are non-relativistic. 
