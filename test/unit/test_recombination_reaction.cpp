...
TEST(Recombination, calc_rate) {
    ...
    auto test_data_calc = DataCalculator<decltype(test_data), decltype(test_data), decltype(test_data_2)>(
        particle_spec, test_data, test_data, test_data_2
    );

    auto test_normalised_potential_energy = 13.6;

    auto test_reaction = Recombination<decltype(test_data), decltype(test_data_calc)>(
        particle_group->sycl_target, Sym<REAL>("TOT_REACTION_RATE"), Sym<REAL>("WEIGHT"),
        test_data, test_data_calc, marker_species,
        Species("ELECTRON"), neutral_species, particle_spec, test_normalised_potential_energy
    );
    ...
    for (int i = 0; i < cell_count; i++) {
        ...
        test_reaction.descendant_product_loop(particle_sub_group, i, i+1, 0.1, descendant_particles);
        ...
        auto id_child = descendant_particles->get_cell(Sym<INT>("INTERNAL_STATE"), i);

        auto target_source_density = particle_group->get_cell(Sym<REAL>("ION_SOURCE_DENSITY"), i);
        auto projectile_source_density = particle_group->get_cell(Sym<REAL>("ELECTRON_SOURCE_DENSITY"), i);

        auto target_source_momentum = particle_group->get_cell(Sym<REAL>("ION_SOURCE_MOMENTUM"), i);
        
        auto target_source_energy = particle_group->get_cell(Sym<REAL>("ION_SOURCE_ENERGY"), i);
        auto projectile_source_energy = particle_group->get_cell(Sym<REAL>("ELECTRON_SOURCE_ENERGY"), i);
        ...
        for (int rowx = 0; rowx < nrow; rowx++) {
            ...
            EXPECT_DOUBLE_EQ(projectile_source_energy->at(rowx, 0), -(0.1 -  (test_normalised_potential_energy * 0.1)));
        }
    }
