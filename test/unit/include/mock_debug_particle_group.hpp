#ifndef REACTIONS_MOCK_TRIM_PARTICLE_GROUP_H
#define REACTIONS_MOCK_TRIM_PARTICLE_GROUP_H
...
inline auto create_trim_test_particle_group(int N_total)
    -> std::shared_ptr<ParticleGroup> {

  auto dims = std::vector<int>(ndim, 1);

  const double cell_extent = 1.0;
  const int subdivision_order = 0;
  const int stencil_width = 1;

  const int pre_subdivision_cells = 1;
