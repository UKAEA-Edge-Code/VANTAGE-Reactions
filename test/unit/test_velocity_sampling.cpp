...
TEST(VelocitySampling, FilteredMaxwellianFailure) {
    struct AlwaysFailCrossSection : public AbstractCrossSection {
      ...
      bool accept_reject(REAL relative_vel = 0.0, REAL uniform_rand = 0.0, REAL value_at = 0.0, REAL max_rate_val = 0.0) const {
        return false;
      }
    };
    ...
    const int ndim = 2;
    auto sampler = FilteredMaxwellianSampler<ndim, decltype(cs)>(
      1.0, cs, rng_kernel
    );
    ...
    auto req_int_props_ = utils::build_sym_vector<INT>(
      particle_group->get_particle_spec(), sampler.get_required_int_props()
    );
  
    auto req_real_props_ = utils::build_sym_vector<REAL>(
      particle_group->get_particle_spec(), sampler.get_required_real_props()
    );
  
  #if NESO_PARTICLES_DEVICE_TYPE_CPU
    particle_loop(
      "vel_sampling_fail_loop", particle_group,
      [=](auto particle_index, auto req_int_props, auto req_real_props, auto kernel) {
        EXPECT_THROW(sampler_on_device.calc_data(
          particle_index, req_int_props, req_real_props, kernel
        ),
        std::runtime_error);
      },
      Access::read(ParticleLoopIndex{}),
      Access::read(sym_vector<INT>(particle_group, req_int_props_)),
