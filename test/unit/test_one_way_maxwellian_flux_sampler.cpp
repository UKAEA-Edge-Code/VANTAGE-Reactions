...
TEST(OneWayMaxwellianFluxSampler, SamplesExpectedVelocityWithDeterministicRNG) {
  ...
  particle_group->add_particle_dat(NP::Sym<NP::REAL>("SURFACE_BASIS_E1"), 3);
  particle_group->add_particle_dat(NP::Sym<NP::REAL>("SURFACE_BASIS_E2"), 3);
  particle_group->add_particle_dat(NP::Sym<NP::REAL>("SURFACE_BASIS_PI"), 3);
  particle_group->add_particle_dat(NP::Sym<NP::REAL>("SAMPLED_VELOCITY"), 3);

  std::array<NP::REAL, 3> flow_speed{};
  ...
  particle_loop(
      ...
      NP::Access::write(NP::Sym<NP::REAL>("FLUID_FLOW_SPEED")),
      NP::Access::write(NP::Sym<NP::REAL>("SURFACE_BASIS_E1")),
      NP::Access::write(NP::Sym<NP::REAL>("SURFACE_BASIS_E2")),
      NP::Access::write(NP::Sym<NP::REAL>("SURFACE_BASIS_PI")))
      ->execute();

  const NP::REAL norm_ratio = 1.0;
  const NP::REAL sampled_e1 =
      (-2.0 * std::sqrt(2.0) + 3.0 * std::sqrt(3.0) + 5.0 * std::sqrt(5.0)) /
      std::sqrt(38.0);
  const NP::REAL sampled_e2 =
      (-5.0 * std::sqrt(2.0) + 7.0 * std::sqrt(3.0) + 11.0 * std::sqrt(5.0)) /
          std::sqrt(195.0) -
      std::sqrt(2.0 * norm_ratio) * std::sqrt(-2 * std::log(0.75));
  const NP::REAL sampled_pi = 3.0 * std::sqrt(2.0 * norm_ratio);

  std::array<NP::REAL, 3> expected_vals;
  ...
  auto rng_lambda = [&]() -> NP::REAL { return 0.75; };
  auto rng_kernel =
  ...
  particle_loop(
      ...
      NP::Access::write(NP::Sym<NP::REAL>("SAMPLED_VELOCITY")),
  ...
  for (int i = 0; i < cell_count; i++) {
    auto sampled_cell =
        particle_group->get_cell(NP::Sym<NP::REAL>("SAMPLED_VELOCITY"), i);
