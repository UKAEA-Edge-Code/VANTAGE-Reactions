...
#include "../../src/reactions/reactions.hpp"
...
TEST(TruncatedMaxwellianSampler, SamplesExpectedVelocityWithDeterministicRNG) {
  ...
  const REAL sampled_e1 = (-2.0*std::sqrt(2.0)+3.0*std::sqrt(3.0)+5.0*std::sqrt(5.0))/std::sqrt(38.0);
  const REAL sampled_e2 = (-5.0*std::sqrt(2.0)+7.0*std::sqrt(3.0)+11.0*std::sqrt(5.0))/std::sqrt(195.0)-std::sqrt(2.0*norm_ratio)*std::sqrt(-2*std::log(0.75));
  const REAL sampled_pi = 3.0*std::sqrt(2.0*norm_ratio);//(-3.0*std::sqrt(2.0)+5.0*std::sqrt(3.0)+7.0*std::sqrt(5.0))/std::sqrt(83.0)+
  ...
  for (int i = 0; i < cell_count; i++) {
    auto sampled_cell = particle_group->get_cell(Sym<REAL>("SAMPLED_VELOCITY"), i);
