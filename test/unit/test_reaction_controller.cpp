...
TEST(DiagnoseReaction, child_transform) {
  const int N_total = 1600;

  auto particle_group = create_test_particle_group(N_total);

  auto test_reaction = TestReaction<1>(particle_group->sycl_target,
                                       Sym<REAL>("TOT_REACTION_RATE"), 5.0, 0,
                                       std::array<int, 1>{1});
  ...
  auto test_parent_wrapper =
      make_shared<TransformationWrapper>(parent_transform);

  auto test_child_wrapper = make_shared<TransformationWrapper>(child_transform);

  auto reaction_controller = ReactionController(
      test_parent_wrapper, test_child_wrapper, Sym<INT>("INTERNAL_STATE"));

  reaction_controller.add_reaction(
      std::make_shared<TestReaction<1>>(test_reaction));
