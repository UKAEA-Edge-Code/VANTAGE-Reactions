...
TEST(ExtrapolationTest, REACTION_DATA_1D_LINEAR_OVER_TYPE_0) {
  ...
  auto ADAS_values = coefficient_values_1D_linear();
...
TEST(ExtrapolationTest, REACTION_DATA_1D_LINEAR_OVER_TYPE_1) {
  ...
  auto prop0_extract = extract<1>("PROP0");
  auto interpolator_data =
      InterpolateData<ndim, 1>(dims_vec, ranges_vec, grid, sycl_target);
...
TEST(ExtrapolationTest, REACTION_DATA_2D_UNDER_OVER_TYPE_2) {
  // Interpolation points
  ...
  const int cell_count = 1; // particle_group->domain->mesh->get_cell_count();
