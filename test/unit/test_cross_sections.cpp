...
TEST(CrossSections, AMJUEL_H1_bulk) {
  ...
  auto cs = AMJUELFitCrossSection<2, 0, 0>(
      vel_norm, 1e-4, mass_amu, std::array<NP::REAL, 2>{-1.0, -0.1},
      std::array<NP::REAL, 0>{}, std::array<NP::REAL, 0>{}, 2.0, 1e4, E_max);
...
TEST(CrossSections, AMJUEL_H1_low_energy) {
  ...
  auto cs = AMJUELFitCrossSection<2, 2, 0>(
      vel_norm, 1e-4, mass_amu, std::array<NP::REAL, 2>{-1.0, -0.1},
      std::array<NP::REAL, 2>{-2.0, -0.2}, std::array<NP::REAL, 0>{}, 2.0, 1e4,
      E_max);
...
TEST(CrossSections, AMJUEL_H1_high_energy) {
  ...
  auto cs = AMJUELFitCrossSection<2, 2, 2>(
      vel_norm, 1e-4, mass_amu, std::array<NP::REAL, 2>{-1.0, -0.1},
      std::array<NP::REAL, 2>{-2.0, -0.2}, std::array<NP::REAL, 2>{-3.0, -0.3},
      2.0, 1e4, E_max);
