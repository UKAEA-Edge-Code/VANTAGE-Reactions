inline void amjuel_2d_example() {
  ...
  // alpha coefficients (the inner array size is the number of density coefficients, the outer is temperature)
  auto coeffs = std::array<std::array<REAL, 2>, 2>{std::array<REAL, 2>{1.0, 0.02},
                                                   std::array<REAL, 2>{0.01, 0.02}};

  // We pass the number of fit coefficients to the constructor as a template parameter
  auto amjuel_data = AMJUEL2DData<2,2>(1.0, // The normalisation of the evolved quantity (density, energy, particle weight, etc.)
