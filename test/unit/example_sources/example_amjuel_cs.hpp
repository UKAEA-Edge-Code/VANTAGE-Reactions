void amjuel_h1_cs_example() {
  ...
  auto coeffs = std::array<NP::REAL, 3>{1.0, 1.0, 1.0}; // a_n coefficients
  auto l_coeffs =
      std::array<NP::REAL, 3>{1.0, 1.0, 1.0}; // left asymptote a_n coefficients
  auto r_coeffs = std::array<NP::REAL, 3>{
      1.0, 1.0, 1.0}; // right asymptote a_n coefficients

  NP::REAL E_lab_max = 1e3;  // energy value after which the r_coeffs are used
  NP::REAL E_lab_min = 1e-1; // energy value after which the l_coeffs are used

  NP::REAL reduced_mass_amu = 1.0;
  ...
  auto cs = AMJUELFitCrossSection<
      ...
      >(1e6,  // velocity normalisation
        ...
        l_coeffs,  // Left asymptote coefficients - set to
                   // std::array<NP::REAL,0>{} if no left asymptote
        r_coeffs,  // Right asymptote coefficents - set to
                   // std::array<NP::REAL,0>{} if no right asymptote
