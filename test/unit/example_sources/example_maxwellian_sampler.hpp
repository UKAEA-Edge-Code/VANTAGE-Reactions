inline void maxwellian_sampler_example() {
  ...
  // In general, it will need the atomic block kernel (see NESO-Particles documentation)
  // This is because rejection sampling has an a priori unknown number of samples
  // 
  // Here we use an arbitrary lambda, but this should in general be a standard uniform distribution
  ...
  auto sampler_data = FilteredMaxwellianSampler<2>(
      ...
      rng_kernel, // RNG kernel used to perform Box-Muller sampling and rejection sampling
