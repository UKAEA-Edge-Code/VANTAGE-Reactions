...
struct DummyDataOnDevice : 
    public ReactionDataBaseOnDevice<1 // This is the dimensionality of the data 
                                      // In general, you would also template against the RNG kernel type if used (unused here)
                                      > {
  ...
  calc_data(const Access::LoopIndex::Read &index, // This is the NESO-Particles index accessor, needed for accessing the particle data
            ...
            typename ReactionDataBaseOnDevice::RNG_KERNEL_TYPE::KernelType
                &kernel // This is the random kernel - unused here - see the FilteredMaxwellianSampler for an example where this is used
                ) const {

    auto weight = req_real_props.at(this->weight_ind, index, 0); // Here we access the required real prop at the weight_ind - the particle weight

    return std::array<REAL, 1>{weight * this->rate}; // The rate is given as weight * rate
  }
...
struct DummyData :
  ...
  DummyData(REAL rate_coefficient,
    ...
    //
    // See the Properties class implementation, as well as the reaction data base classes
...
public:
  ...
  // We need to provide this accessor so that the object can be retrieved for use in loops
  DummyDataOnDevice get_on_device_obj() {
