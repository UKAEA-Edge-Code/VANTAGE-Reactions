inline void linear_reaction_CX_example(ParticleGroupSharedPtr particle_group) {
  ...
  auto mark_id_zero =
      make_marking_strategy<ComparisonMarkerSingle<INT, EqualsComp>>(
          Sym<INT>(prop_map[default_properties.internal_state]), // this will result in "INTERNAL_STATE" when using the default map
  ...
  auto rate_data = FixedRateData(1.0); // the values used here are somewhat arbitrary, 
                                       // normally they would depend on the normalisation

  // Since the CX kernels require ndim values per particle in additional reaction data
  // we build a DataCalculator that will produce ndim values
  //
  ...
  //
  // For this example, we mimic a beam in 2D, by using two FixedRateData objects in the 
  // DataCalculator
  //
  ...
  // as well as the two species 
  
  auto cx_kernel = CXReactionKernels<2>(target_species,
                                        projectile_species,
                                        prop_map // The property map used to remap any of the required properties in the kernel
                                                 // The CX kernel requires default_properties.weight, default_properties.velocity
                                                 // as well as some of the source properies (see the CXReactionKernels docs)
                                        );

  // We can now assemble the linear reaction using the base class constructor
  auto cx_reaction =
      LinearReactionBase<1, // The number of outgoing particles - CX will produce one neutral of the target species 
                         FixedRateData, // The reaction data class to be used for the rate calculation
  ...
  auto product_group = std::make_shared<ParticleGroup>(
      particle_group->domain, particle_spec,
  ...
  for (int i = 0; i < cell_count; i++) {
    ...
    cx_reaction.run_rate_loop(particle_sub_group, i);
    ...
    cx_reaction.descendant_product_loop(particle_sub_group, i, 0.1,
                                        product_group);
